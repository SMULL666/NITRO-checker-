...
module.exports = {
	...
	checkForUpdates: (silent = false) => {
		if (module.exports.updateAvailable) {
			if (silent) return;
			return logger.info(chalk.bold(`An update is available on GitHub (v${module.exports.updateAvailable}) ! ${chalk.blue('https://github.com/Tenclea/ReSMULL')}`));
		}

		(async () => {
			const res = await needle('get', 'https://raw.githubusercontent.com/Tenclea/ReSMULL/main/package.json')
			...
			if (version !== update) {
				...
				if (!silent) return logger.info(chalk.bold(`An update is available on GitHub (v${module.exports.updateAvailable}) ! ${chalk.blue('https://github.com/Tenclea/ReSMULL')}`));
			}
		})();
	},
	...
	sendWebhook: (url, message) => {
		...
		const data = JSON.stringify({ 'username': 'ReSMULL', 'avatar_url': 'https://cdn.discordapp.com/attachments/794307799965368340/794356433806032936/20210101_010801.jpg', 'content': message });
	...
	validateProxies: async (p) => {
		const res = await needle(
			'post',
			'https://SMULLdb.tenclea.repl.co/proxies',
