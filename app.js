...
console.log(chalk.blue(`\u001B[?25l
	...
	███████ ██      ██  ██████  ███████ ███████ 
               ...
               ${chalk.italic.gray(`v${stats.version} - by SMULL`)}
`));
...
process.on('exit', () => { logger.info('Closing SMULL... If you liked this project, make sure to leave it a star on github: https://github.com/SMULL666 <3'); checkForUpdates(); });
...
(async () => {
	...
	const logStats = () => {
		...
		process.title = `SMULL - by SMULL | Proxies: ${proxies.length + stats.threads} | Attempts: ${attempts} (~${aps.toFixed(0)}/min) | Working Codes: ${stats.working}`;
		return;
	};
